  ...
  try {
    const promises = STOCKS.map(async (stock) => {
      try {
        ...
        const fallback = FALLBACK_PRICES[stock.symbol];
        return { ...stock, ...fallback, timestamp: now } as CachedPrice;
      }
    });
...
export default async function handler(req: any, res: any) {
  try {
    ...
    // Stock APIs
    if (req.url === "/api/stocks/prices" && req.method === "GET") {
      try {
        const prices = await fetchStockPrices();
        return res.status(200).json({ stocks: prices });
      } catch (error) {
        return res.status(500).json({ message: "Failed to fetch stock prices" });
      }
    }

    if (req.url?.startsWith("/api/stocks/quote/") && req.method === "GET") {
      try {
    ...
    if (req.url === "/api/stocks/list" && req.method === "GET") {
